...
bool isBalanced(const BST<KT, VT>& bst) {
	...
	if(abs(treeHeight(bst.root->left) - treeHeight(bst.root->right)) > 1) return false;
	if(!isBalanced(bst.root->left)) return false;
	if(!isBalanced(bst.root->right)) return false;
...
int diameter(const BST<KT, VT>& bst) {
	...
	if(bst.is_empty()) return 0;
	int left_diameter = diameter(bst.root->left);
...
char decode(const BST<int, char>& bst, const string& symbol, unsigned int index = 0) {
	...
	else return decode(bst.root->right, symbol, index + 1);
