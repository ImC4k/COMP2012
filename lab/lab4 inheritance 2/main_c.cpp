...
int main()
{
	...
	consultantPtr = econ; // slicing occurs, consultantPtr cannot have EducationalConsultant's functions/ members
	consultantPtr->check(Item::id);
	consultantPtr->check(Item::name);
	// consultantPtr->setWage(650); // BUG SLICING
	// consultantPtr->setHours(90); // BUG SLICING
	consultantPtr->check(Item::base_salary);
	consultantPtr->check(Item::performance_salary);
	consultantPtr->check(Item::net_salary); // BUG, mismatch of net_salary
	cout<<endl;

	Employee& employeeRef = *econ;
	employeeRef.check(Item::id);
	employeeRef.check(Item::name);
	// employeeRef.setBaseSalary(20000); // BUG SLICING
