...
Matrix::Matrix(Matrix &&mat) {
  ...
  // NOTE: must turn mat.ptr to nullptr, otherwise, ptr will be destructed when temp obj is killed
  *this = mat;
}
...
Matrix Matrix::operator+(const Matrix &mat) {
  if (print_step) cout << "operator +" << endl;     // please keep this line
  Matrix newObj(this->num_rows, this->num_cols);
  for(int i = 0; i < num_rows*num_cols; i++){
    newObj.ptr[i] = this->ptr[i] + mat.ptr[i];
  }
  return newObj;
}
